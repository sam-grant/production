...
//    Fill2D("verticalPos_vs_time",tr->trackT0,tr->decayVertexPosY);
    ...
    double R = sqrt(pow(ttc->decayVertexPosX,2)+pow(ttc->decayVertexPosZ,2))-7112;
    double p = sqrt(pow(ttc->decayVertexMomX,2)+pow(ttc->decayVertexMomY,2)+pow(ttc->decayVertexMomZ,2));
    ...
    Fill2D("radialPos_vs_time",ttc->decayTime, R);
