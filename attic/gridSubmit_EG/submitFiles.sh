...
pnfsOutDir=/pnfs/GM2/scratch/users/${USER}/trackAndTrackCaloTrees_EG/${run}
...
while read line; do 
  ...
  if [ -f ${pnfsOutDir}/trackAndTrackCaloTrees_${run}.${subRun}.root ]; then 
    echo "ERROR: ${pnfsOutDir}/trackCaloTree_${run}.${subRun}.root already exists and would be overwritten."
    return
  fi
done < FileList.txt
...
if [ -f ${pnfsOutDir}/libgm2tracker_analyses_TrackCaloTreeMaker_module.so ]; then
...
cat <<EOF > runTrackCaloMatchingTreeMaker${fileNum}.sh
...
if [ -f ${pnfsOutDir}/runTrackCaloMatchingTreeMaker${fileNum}.sh ]; then
  rm -f ${pnfsOutDir}/runTrackCaloMatchingTreeMaker${fileNum}.sh
fi
  ifdh cp runTrackCaloMatchingTreeMaker${fileNum}.sh ${pnfsOutDir}/runTrackCaloMatchingTreeMaker${fileNum}.sh
while [ ! -f ${pnfsOutDir}/runTrackCaloMatchingTreeMaker${fileNum}.sh ]; do
  echo "${pnfsOutDir}/runTrackCaloMatchingTreeMaker${fileNum}.sh not found after ifdh cp.  Sleeping 5..."
  sleep 5
done

#Submit grid job
jobsub_submit -N 1 -G gm2 --OS=SL6 --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC --expected-lifetime=4h --role=Analysis file://${pnfsOutDir}/runTrackCaloMatchingTreeMaker${fileNum}.sh
rm -f runTrackCaloMatchingTreeMaker${fileNum}.sh
