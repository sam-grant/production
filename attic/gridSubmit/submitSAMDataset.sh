  ...
  if ! grep -Fxq "${run} ${subrun}" OutOfSyncSubRuns_5040A_goldList.txt ; then
    if ! grep -Fxq "${run} ${subrun}" BadTrackerSubRuns_5040A_goldList.txt ; then
      if ! grep -Fxq "${run} ${subrun}" DifferentDQCSubRuns_5040A_5042A_goldList.txt ; then
        if [ ! -f /pnfs/GM2/scratch/users/${USER}/TrackingTrees/${run}/trackRecoTrees_${run}.${subrun}.root ]; then
          if [ -f /pnfs/GM2/scratch/users/${USER}/TrackingTrees/${run}/trackRecoTrees_${run}.${subrun}.log ]; then
            echo "!!! /pnfs/GM2/scratch/users/${USER}/TrackingTrees/${run}/trackRecoTrees_${run}.${subrun}.log exists.  Delete all log files first!"
	    return
	  fi
