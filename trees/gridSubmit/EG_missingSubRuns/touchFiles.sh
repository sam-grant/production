...
dataset=$1
pnfsOutDir=/pnfs/GM2/scratch/users/${USER}/TrackAndTrackCaloTrees_EG
  ...
  if ! grep -Fxq "${run} ${subrun}" OutOfSyncSubRuns.txt ; then
    if ! grep -Fxq "${run} ${subrun}" BadTrackerSubRuns.txt ; then
      if ! grep -Fxq "${run} ${subrun}" DifferentDQCSubRuns.txt ; then
        echo Running touch on ${line}
        touch ${pnfsOutDir}/${run}/trackAndTrackCaloTrees_${run}.${subrun}.root
