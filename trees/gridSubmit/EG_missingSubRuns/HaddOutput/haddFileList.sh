...
for line in `cat FileList_${run}.txt`; do
  subrun=${line##*${run}.}
  subrun=${subrun%%.*}
  files=$files" "/pnfs/GM2/scratch/users/sgrant/TrackAndTrackCaloTrees_EG_missingSubRuns/${run}/trackAndTrackCaloTrees_${run}.${subrun}.root
done

# Hadd these files (using Joe's script that does a few at a time)
../../hadd.sh trackAndTrackCaloTrees_${run}.root $files

mv trackAndTrackCaloTrees_${run}.root /gm2/data/g2be/Production/TrackAndTrackCaloTrees/Run1/EG_missingSubRuns/trackAndTrackCaloTrees_${run}.root
