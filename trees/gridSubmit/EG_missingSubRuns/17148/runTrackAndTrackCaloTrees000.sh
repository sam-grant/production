...
while read line; do 
  ...
  gm2 -c RunTrackAndTrackCaloNTup.fcl -T trackAndTrackCaloTrees_17148.${subRun}.root

  # Only copy back if job completed successfully (output from last command was 0)
  if [ $? -eq 0 ]; then
    if [ -f trackAndTrackCaloTrees_17148.${subRun}.root ]; then
      ifdh mv trackAndTrackCaloTrees_17148.${subRun}.root /pnfs/GM2/scratch/users/sgrant/TrackAndTrackCaloTrees_EG_missingSubRuns/17148/trackAndTrackCaloTrees_17148.${subRun}.root
    else 
      echo "trackAndTrackCaloTrees_17148.${subRun}.root does not exist.  ls reads:"
      ls
    fi
  fi
