...
while read line; do 
  ...
  gm2 -c RunTrackAndTrackCaloNTup.fcl -T trackAndTrackCaloTrees_17295.${subRun}.root

  # Only copy back if job completed successfully (output from last command was 0)
  if [ $? -eq 0 ]; then
    if [ -f trackAndTrackCaloTrees_17295.${subRun}.root ]; then
      ifdh mv trackAndTrackCaloTrees_17295.${subRun}.root /pnfs/GM2/scratch/users/sgrant/TrackAndTrackCaloTrees_EG/17295/trackAndTrackCaloTrees_17295.${subRun}.root
    else 
      echo "trackAndTrackCaloTrees_17295.${subRun}.root does not exist.  ls reads:"
      ls
    fi
  fi
