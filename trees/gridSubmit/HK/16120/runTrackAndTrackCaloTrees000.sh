...
while read line; do 
  ...
  gm2 -c RunTrackAndTrackCaloNTup.fcl -T trackAndTrackCaloTrees_16120.${subRun}.root

  # Only copy back if job completed successfully (output from last command was 0)
  if [ $? -eq 0 ]; then
    if [ -f trackAndTrackCaloTrees_16120.${subRun}.root ]; then
      ifdh mv trackAndTrackCaloTrees_16120.${subRun}.root /pnfs/GM2/scratch/users/sgrant/TrackAndTrackCaloTrees_HK/16120/trackAndTrackCaloTrees_16120.${subRun}.root
    else 
      echo "trackAndTrackCaloTrees_16120.${subRun}.root does not exist.  ls reads:"
      ls
    fi
  fi
