...
pnfsOutDir=/pnfs/GM2/scratch/users/${USER}/TrackAndTrackCaloTrees_manualDQC_missingSubRuns/${run}
...
if [ -f ${pnfsOutDir}/libgm2tracker_reco_TrackCaloMatching_module.so ]; then
  rm -f ${pnfsOutDir}/libgm2tracker_reco_TrackCaloMatching_module.so
fi
...
ifdh cp libgm2tracker_reco_TrackCaloMatching_module.so ${pnfsOutDir}/libgm2tracker_reco_TrackCaloMatching_module.so
...
echo "Copied RunTrackAndTrackCaloNTup.fcl, libgm2tracker_filters_ManualDQCFilter_module.so, libgm2tracker_reco_TrackCaloMatching_module.so & libgm2analyses_strawtracker_TrackAndTrackCaloNTup_module.so to $pnfsOutDir"
...
if [ ! -z "\`ifdh ls ${pnfsOutDir}/libgm2tracker_reco_TrackCaloMatching_module.so\`" ]; then
  ifdh cp ${pnfsOutDir}/libgm2tracker_reco_TrackCaloMatching_module.so ./libgm2tracker_reco_TrackCaloMatching_module.so
else 
  echo "${pnfsOutDir}/libgm2tracker_reco_TrackCaloMatching_module.so does not exist"
fi
