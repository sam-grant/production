...
for line in `cat ManualDQCSubRunList.txt`; do
  ...
  if ! grep -Fxq "${run} ${subrun}" BadTrackerSubRuns_5040A_goldList.txt ; then
    if [ ! -f /pnfs/GM2/scratch/users/sgrant/TrackAndTrackCaloTrees_EG_manualDQC/${run}/trackAndTrackCaloTrees_${run}.${subrun}.root ]; then
      if [ -f /pnfs/GM2/scratch/users/sgrant/TrackAndTrackCaloTrees_EG/${run}/trackAndTrackCaloTrees_${run}.${subrun}.log ]; then
        echo "!!! /pnfs/GM2/scratch/users/sgrant/TrackAndTrackCaloTrees_EG/${run}/trackAndTrackCaloTrees_${run}.${subrun}.log exists.  Delete all log files first!"
	      return
      fi
