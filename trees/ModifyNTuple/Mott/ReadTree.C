void ReadTree() { 
  ...
  TFile *file = TFile::Open("newTree.root");
  //TFile *file = TFile::Open("oldTree.root");

  TTree *tree = (TTree*)file->Get("trackAndTrackCalo/tree"); // Seems to have changed to just "tree" rather than "trackAndTrackCalo/tree"
